// O(nlogk), each node is added and deleted once each.
// TreeSet size = k. Add/remove/contains of TreeSet takes O(logK) time.
/*
...
public class Solution {
    public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
        ...
        for (int ind = 0; ind < nums.length; ind++) {
            final Integer floor = values.floor((nums[ind] > Integer.MAX_VALUE - t) ? Integer.MAX_VALUE : nums[ind] + t);
            final Integer ceil = values.ceiling((nums[ind] < Integer.MIN_VALUE + t) ? Integer.MIN_VALUE : nums[ind] - t);
